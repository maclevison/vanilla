...
4. **Review & audit** — two passes, both before merge. Invoke `vanilla-review` for the *taste* half: craft bar + family test (does it read as PDM?) + uniqueness test (signature, or could it be any PDM product?). Invoke `vanilla-audit` for the *measurable* half: WCAG contrast on both themes (via `references/contrast.mjs`), token fidelity, responsive/touch, complete states, family-mechanical. A build ships only when both clear.
