...
- **D · Composition & rhythm** — does it breathe unevenly, or is it monotone (same card, gap, density everywhere)? Do proportions state a relationship? **Shell fidelity:** does the app frame match the brief's *Shell & navigation* — the named archetype (build notes in `references/shells.md`) and its details: the right sidebar / top bar / page shape, the sidebar sharing the canvas with a hairline (not a different fill), the active nav item quiet-but-clear, the top bar holding only what the brief listed? A shell that contradicts the brief (a sidebar it didn't ask for, chrome competing with the focal point) is a finding. The archetype is structure only — same archetype must not mean same-looking product; if it does, that's the soul lens (G), not this one.
