...
- **D · Composition & rhythm** — does it breathe unevenly, or is it monotone (same card, gap, density everywhere)? Do proportions state a relationship? **Shell fidelity:** does the app frame match the brief's *Shell & navigation* — the right sidebar / top bar / page shape, the sidebar sharing the canvas with a hairline (not a different fill), the active nav item quiet-but-clear, the top bar holding only what the brief listed? A shell that contradicts the brief (a sidebar it didn't ask for, chrome competing with the focal point) is a finding.
