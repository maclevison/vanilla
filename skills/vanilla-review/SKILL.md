...
- **C · Surfaces & depth** — quiet elevation (the surface ladder), findable-not-harsh hairlines, one committed depth strategy? Is the **sidebar/chrome a tone distinct from the workspace** (`--vanilla-chrome`, never the same fill)? Are surface transitions **intentional** — no abrupt, jarring background-color jump between adjacent regions; do white cards **float softly** on the canvas rather than sit on a same-value slab? Does each card carry **one** elevation language — a hairline **or** a shadow token, never both (the ghost-card)?
- **D · Composition & rhythm** — does it breathe unevenly, or is it monotone (same card, gap, density everywhere)? Do proportions state a relationship? **Shell fidelity:** does the app frame match the brief's *Shell & navigation* — the named archetype (build notes in `references/shells.md`) and its details: the right sidebar / top bar / page shape, the sidebar on `--vanilla-chrome` (distinct from the workspace canvas, never the same fill) separated by a hairline, the active nav item a **perceptible neutral fill** (`--vanilla-fill-selected`) + weight + lavender icon (**never a side-stripe**), the top-bar controls quiet (ghost icon buttons, low-key context switcher) and holding only what the brief listed? A shell that contradicts the brief (a sidebar it didn't ask for, chrome competing with the focal point) is a finding. The archetype is structure only — same archetype must not mean same-looking product; if it does, that's the soul lens (G), not this one.
...
**Presumptive blockers** (unless justified against the brief): no focal point · size-only or defaulted hierarchy · monotone layout · harsh or fragmented surfaces · **abrupt background-color jump** between adjacent regions (unintentional surface transition) · **sidebar painted the same fill as the workspace** (chrome must be distinct) · **side-stripe / left-accent bar** on a nav item, row, or callout · **ghost-card** (a 1px border and a diffuse shadow on the same element) · **invisible interactive state** (hover/selected that doesn't read, or a surface step that vanishes on a same-value background) · missing states · structural hacks · an inaccessible hand-rolled control · **component blob** (accessories that don't recede, interactives with no separation, a value glued to its label — fails at reading distance) · **skin violation** (hardcoded palette/font, a styled UI kit, non-Lucide icons) · **feel-breaking motion** (`ease-in` on UI, `scale(0)` entry, animation on a keyboard/100+×/day action, a non-GPU animation with an easy GPU fix) · **missing signature** (generic — could be any product). Any one present → **not approved**; leave explicit, actionable feedback.
